...
function renderStoreLocator(block, stores, bridge) {
  ...
  if (!stores || stores.length === 0) {
    ...
    pinIcon.innerHTML = `<svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    button.style.cssText = `background: ${PALETTE[0]}; color: #fff;`;
    if (bridge) {
    ...
    stores.slice(0, 2).forEach((store) => {
      ...
      phone.style.cssText = `color: ${PALETTE[0]};`;
